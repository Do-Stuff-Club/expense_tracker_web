...
context('User Login', () => {
    ...
    it('successfully logs in given corret login info', () => {
        ...
        .type('potato@potato.com').should('have.value', 'potato@potato.com');

        cy.findByLabelText('Password', { timeout: 7500 })
        .type('asdfasdf').should('have.value', 'asdfasdf');
    
        // we have code that clicks the login button
        cy.get('button[type=submit]').findByText('Log In', { timeout: 7500 }).click();

        // https://on.cypress.io/wait
        // our request is successful since we used correct user info
        cy.wait('@postLoginResponse').its('response.statusCode').should('be.oneOf', [200])
